 ...
 *
 */
public class FrameCamSimulator  extends SimpleApplication {
    ...
    {
        double seconds = 6.5*10;
        double fps = 100;
        
        FrameCamSimulator app = new FrameCamSimulator(true, seconds, fps);
        
        app.start();
        app.waitUntilEnd();
        
        app = new FrameCamSimulator(false, seconds, fps);
        app.start();
    }
    private EventWriterPostProcessor writer;
    

    public FrameCamSimulator(boolean left, double stopTime, double fps)
    {
        showSettings = false;
        AppSettings settings = new AppSettings(true);
        settings.setResolution(320*2, 240*2);
...
//        setTimer(manualTimer);
    ...
    public void update()
    {
        ...
        if (getTimer().getTimeInSeconds() >= stopTime)
        {
            stop();
        }
    }
    ...
    public void setupEventWriter()
    {
        ...
        {
            Logger.getLogger(FrameCamSimulator.class.getName()).log(Level.SEVERE, null, ex);
        }
